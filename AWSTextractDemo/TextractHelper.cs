...
namespace AWSTextractDemo
{
    internal class TextractHelper
    {
        ...
        public async Task AnalyzeForm(string filename)
        {
            // Upload document to S3.
            ...
            Console.WriteLine("Starting document analysis job");
            var startJobRequest = new StartDocumentAnalysisRequest { DocumentLocation = docLocation, FeatureTypes = { "FORMS" } };
            var startJobResponse = await _textractClient.StartDocumentAnalysisAsync(startJobRequest);
            Console.WriteLine($"Job ID: {startJobResponse.JobId}");
